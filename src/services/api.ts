...
//   // Handle case where response is an array of work orders (legacy support)
...
//   console.log('ApiService: WorkOrder Mechanics:', JSON.stringify(response.workOrder.mechanics, null, 2));
//   console.log('ApiService: WorkOrder Shop Managers:', JSON.stringify(response.workOrder.shopManager, null, 2));
//   return {
...
class ApiService {
  private request = async (endpoint: string, options: RequestInit = {}) => {
    const url = `${API_BASE_URL}${endpoint}`;
    const headers = {
      // Only include Content-Type for non-FormData requests
    ...
    if (options.body instanceof FormData) {
      console.log(`FormData for ${endpoint}:`);
  ...
  deleteMechanicFromWorkOrder = async (workOrderId: string, mechanicId: string) => {
    return this.request(`/workorder/${workOrderId}/delete-mechanic-from-workorder/${mechanicId}`, {
      method: 'DELETE',
    });
  };

  deleteManagerFromWorkOrder = async (workOrderId: string, managerId: string) => {
    return this.request(`/workorder/${workOrderId}/delete-manager-from-workorder/${managerId}`, {
      method: 'DELETE',
    });
  };
  createRepair = async (dto: CreateRepairDto, beforeImage?: File, afterImage?: File): Promise<Repair> => {
