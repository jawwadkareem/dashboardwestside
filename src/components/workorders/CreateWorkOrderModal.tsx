...
import { CreateWorkOrderDto, Car, ApiError } from '../../types';
...
export const CreateWorkOrderModal: React.FC<CreateWorkOrderModalProps> = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const user = authUtils.getUser();
      if (!user) throw { message: 'User not authenticated', statusCode: 401 };
      const workOrder = await apiService.createWorkOrder({ ...formData, createdBy: user._id });
      setToast({ message: 'Work order created successfully', type: 'success' });
      onSuccess(workOrder);
    } catch (error: any) {
      setToast({ message: error.message || 'Failed to create work order', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
